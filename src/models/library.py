...
class Book(Base):
    ...
    accession_number = Column(String(100), nullable=False, index=True)
...
class BookIssue(Base):
    ...
    returned_to = Column(Integer, ForeignKey('users.id', ondelete='SET NULL'), nullable=True, index=True)
    ...
    fine_payment_date = Column(Date, nullable=True)
...
class LibrarySettings(Base):
    ...
    fine_per_day = Column(Numeric(10, 2), default=1, nullable=False)
    max_fine_amount = Column(Numeric(10, 2), nullable=True)
    allow_renewals = Column(Boolean, default=True, nullable=False)
    max_renewals = Column(Integer, default=2, nullable=False)
    working_days = Column(String(50), default="mon,tue,wed,thu,fri", nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)
