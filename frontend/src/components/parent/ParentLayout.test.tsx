...
const TestDashboard = () => <div>Parent Dashboard Content</div>;
const TestProgress = () => <div>Parent Progress Content</div>;

const ParentLayoutWithRoutes = () => (
  <MemoryRouter initialEntries={['/parent/dashboard']}>
    <Routes>
      <Route path="/parent" element={<ParentLayout />}>
        <Route path="dashboard" element={<TestDashboard />} />
        <Route path="progress" element={<TestProgress />} />
      </Route>
    </Routes>
  </MemoryRouter>
);

describe('Parent Layout - Mobile Sidebar Tests (<600px viewport)', () => {
  beforeEach(() => {
    setupDemoParent();
  ...
  describe('Hamburger Menu Button', () => {
    ...
    it('should open mobile drawer when hamburger is clicked', async () => {
      ...
      await waitFor(() => {
        const drawer = screen.getByRole('navigation', {
          name: /mobile navigation|main navigation/i,
        });
    ...
    it('should toggle drawer state multiple times', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        await waitFor(() => {
          expect(
            screen.getByRole('navigation', { name: /mobile navigation|main navigation/i })
          ).toBeInTheDocument();
        });
