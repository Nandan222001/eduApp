...
import { setupDemoTeacher } from '../../../tests/setup';
...
const TestDashboard = () => <div>Teacher Dashboard Content</div>;
const TestClasses = () => <div>Teacher Classes Content</div>;

const TeacherLayoutWithRoutes = () => (
  <MemoryRouter initialEntries={['/teacher/dashboard']}>
    <Routes>
      <Route path="/teacher" element={<TeacherLayout />}>
        <Route path="dashboard" element={<TestDashboard />} />
        <Route path="classes" element={<TestClasses />} />
      </Route>
    </Routes>
  </MemoryRouter>
);

describe('Teacher Layout - Mobile Sidebar Tests (<600px viewport)', () => {
  beforeEach(() => {
    setupDemoTeacher();
  ...
  describe('Hamburger Menu Button', () => {
    it('should render hamburger menu button in mobile viewport', () => {
      renderWithProviders(<TeacherLayoutWithRoutes />, { withRouter: false });
    ...
    it('should close mobile drawer when hamburger is clicked again', async () => {
      ...
      await waitFor(
        () => {
          const drawer = screen.queryByRole('navigation', { name: /mobile navigation|main navigation/i });
    ...
    it('should toggle drawer state multiple times', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        await user.click(hamburgerButton);
        await waitFor(() => {
          expect(screen.getByRole('navigation', { name: /mobile navigation|main navigation/i })).toBeInTheDocument();
        });
        ...
        await waitFor(
          () => {
            expect(
              screen.queryByRole('navigation', { name: /mobile navigation|main navigation/i })
  ...
  describe('Navigation Items', () => {
    ...
    it('should display Teacher Portal subtitle', async () => {
      const user = userEvent.setup();
      renderWithProviders(<TeacherLayoutWithRoutes />, { withRouter: false });
      ...
      await waitFor(() => {
        const drawer = screen.getByRole('navigation', { name: /mobile navigation|main navigation/i });
        expect(within(drawer).getByText('Teacher Portal')).toBeInTheDocument();
      });
    });
    ...
    it('should render teacher-specific navigation items', async () => {
      const user = userEvent.setup();
      renderWithProviders(<TeacherLayoutWithRoutes />, { withRouter: false });
